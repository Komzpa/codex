...
fn spawn_agent_common_properties_v2(agent_type_description: &str) -> BTreeMap<String, JsonSchema> {
    BTreeMap::from([
        ...
        (
            ...
            JsonSchema::string(Some(format!(
                "Optional number of turns to fork. Defaults to the most recent {DEFAULT_MULTI_AGENT_V2_FORK_TURNS} turns. Use `none`, `all`, or a positive integer string such as `3`."
            ))),
        ),
        (
            "model".to_string(),
