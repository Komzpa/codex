...
"#;
const DEFAULT_MULTI_AGENT_V2_MODEL_OVERRIDE_USAGE_HINT_TEXT: &str = "Only explicit full-history forks (`fork_turns=\"all\"`) inherit the parent model and reasoning effort and reject overrides. Omitting `fork_turns` keeps the most recent 3 turns. Only set `model` or `reasoning_effort` when explicitly requested by the user, applicable `AGENTS.md` instructions, or skill instructions.";
