...
"#;
const DEFAULT_MULTI_AGENT_V2_MODEL_OVERRIDE_USAGE_HINT_TEXT: &str = "Bounded and no-history forks begin with the parent model and reasoning effort but accept explicit overrides. Full-history forks (`fork_turns=\"all\"`) inherit those settings and reject overrides. Omitting `fork_turns` keeps the most recent 3 turns. Only set `model` or `reasoning_effort` when explicitly requested by the user, applicable `AGENTS.md` instructions, or skill instructions.";
...
impl MultiAgentV2Config {
    fn defaults_for_max_concurrency(max_concurrent_threads_per_session: usize) -> Self {
        Self {
            ...
            usage_hint_text: Some(
                DEFAULT_MULTI_AGENT_V2_MODEL_OVERRIDE_USAGE_HINT_TEXT.to_string(),
            ),
