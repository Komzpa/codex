...
pub(crate) fn reattach_latest_complete_tool_transaction_annotated(
    compacted_history: Vec<ResponseItemEnvelope>,
    trace_input_history: &[ResponseItem],
    context_window: Option<i64>,
    base_instructions: &BaseInstructions,
) -> CodexResult<Vec<ResponseItemEnvelope>> {
    let raw_history = compacted_history
        .iter()
        .map(|envelope| envelope.item.clone())
        .collect::<Vec<_>>();
    let reattached = reattach_latest_complete_tool_transaction(
        raw_history,
        trace_input_history,
        context_window,
        base_instructions,
    )?;
    let mut remaining = compacted_history;
    Ok(reattached
        .into_iter()
        .map(|item| {
            remaining
                .iter()
                .position(|envelope| envelope.item == item)
                .map(|index| remaining.remove(index))
                .unwrap_or_else(|| ResponseItemEnvelope::new(item))
        })
        .collect())
}
